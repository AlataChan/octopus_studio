...
export default function UserSetup({ setHeader, setForwardBtn, setBackBtn }) {
  ...
  return (
    <div className="w-full flex items-center justify-center flex-col gap-y-6">
      <div className="flex flex-col border rounded-lg border-theme-border-medium light:border-theme-sidebar-border p-8 items-center gap-y-4 w-full max-w-[600px]">
...
const JustMe = ({
  ...
  const handleSubmit = async (e) => {
    ...
    // 移除自动登录逻辑，让用户在 Onboarding 完成后手动登录
    // 这样用户可以体验完整的登录流程，并验证自己的密码
    showToast("密码设置成功！Onboarding 完成后请使用密码登录", "success", {
      autoClose: 5000,
    });
      ...
      <div className="flex flex-col border rounded-lg border-theme-border-medium light:border-theme-sidebar-border p-8 items-center gap-y-4 w-full max-w-[600px]">
        ...
        {enablePassword && (
          <form className="w-full mt-4" onSubmit={handleSubmit}>
            ...
            />
            <div className="mt-4 text-theme-text-primary text-opacity-80 text-xs font-base -mb-2">
...
const MyTeam = ({ setMultiUserLoginValid, myTeamSubmitRef, navigate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // 移除自动登录逻辑，让用户在 Onboarding 完成后手动登录
    // 这样用户可以体验完整的登录流程，并验证自己的用户名和密码
    showToast(
      `Admin 用户 "${data.username}" 创建成功！Onboarding 完成后请登录`,
      "success",
      { autoClose: 5000 }
    );
  ...
  return (
    <div className="w-full flex items-center justify-center border max-w-[600px] rounded-lg border-theme-border-medium light:border-theme-sidebar-border">
