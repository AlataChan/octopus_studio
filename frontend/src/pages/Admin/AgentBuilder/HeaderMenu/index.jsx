...
import AnythingInfinityLogo from "@/media/logo/octopus-studio-icon-orange.png";
      ...
      <div className="flex justify-between items-start max-w-[1700px] mx-auto">
        <div className="flex items-center gap-x-2">
          <button
            ...
            className="w-8 h-8 flex items-center justify-center rounded-full bg-theme-settings-input-bg border border-theme-border hover:bg-theme-action-menu-bg transition-colors duration-300"
          >
            <CaretLeft
              weight="bold"
          ...
          <div
            className="flex items-center bg-theme-settings-input-bg rounded-md border border-theme-border pointer-events-auto"
            ref={dropdownRef}
          >
            <button
              onClick={() => navigate(paths.settings.agentSkills())}
              className="!border-t-transparent !border-l-transparent !border-b-transparent flex items-center gap-x-2 px-4 py-2 border-r border-theme-border hover:bg-theme-action-menu-bg transition-colors duration-300"
            >
              <img
            ...
            <div className="relative">
              ...
              {showDropdown && (
                <div className="absolute top-full left-0 mt-1 w-full min-w-[200px] max-w-[350px] bg-theme-settings-input-bg border border-theme-border rounded-md shadow-lg z-50 animate-fadeUpIn">
                  {availableFlows
        ...
        <div className="flex flex-col gap-y-1 items-end">
          <div className="flex items-center gap-x-[15px]">
            ...
            {typeof onSaveAsSkillTemplate === "function" && (
              <button
                onClick={onSaveAsSkillTemplate}
                className="px-3 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2 border border-theme-border bg-theme-settings-input-bg text-theme-text-primary hover:bg-theme-action-menu-bg transition-all duration-300"
              >
                保存为 Skill 模板
              </button>
            )}
            <button
              ...
              className="px-3 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2 border border-theme-border bg-theme-bg-primary text-theme-text-primary hover:bg-theme-action-menu-bg transition-all duration-300"
            >
              发布
            </button>
            <button
              onClick={onSaveFlow}
              className="border-none bg-primary-button hover:opacity-80 text-black light:text-theme-text-primary px-3 py-2 rounded-lg text-sm font-medium transition-all duration-300 flex items-center justify-center gap-2"
            >
              保存
            </button>
          </div>
          <Link
            to="https://github.com/AlataChan/octopus_studio.git"
