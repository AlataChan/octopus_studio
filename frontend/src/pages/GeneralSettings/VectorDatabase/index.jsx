...
const UNAVAILABLE_VECTOR_DB = {
  name: "Unavailable vector database",
  value: "unavailable",
  logo: OctopusStudioIcon,
  options: null,
  description:
    "The saved vector database provider is no longer available. Choose a supported provider to continue.",
};
...
export default function GeneralVectorDatabase() {
  ...
  useEffect(() => {
    if (!settings) return;
    setSelectedVDB((current) => current ?? settings?.VectorDB ?? "lancedb");
  ...
  const VECTOR_DBS = [
    {
      ...
      description:
        "100% local vector DB that runs on the same instance as Octopus Studio.",
    },
    {
      name: "PGVector",
      value: "pgvector",
  ...
  const selectedVDBObject =
    ...
    UNAVAILABLE_VECTOR_DB;
  const selectedVDBAvailable = VECTOR_DBS.some(
        ...
        >
          ...
          >
            <div className="flex flex-col w-full px-1 md:pl-6 md:pr-[50px] py-16 md:py-6">
              ...
              >
                {selectedVDBAvailable && selectedVDBObject.options}
              </div>
            </div>
          </form>
        </div>
