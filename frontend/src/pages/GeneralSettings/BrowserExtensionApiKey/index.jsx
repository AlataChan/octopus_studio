        ...
        <div className="flex flex-col w-full px-1 md:pl-6 md:pr-[50px] md:py-6 py-16">
          <div className="w-full flex flex-col gap-y-1 pb-6 border-theme-border border-b-2">
            <div className="items-center flex gap-x-4">
              <p className="text-lg leading-6 font-bold text-theme-text-primary">
                {t("browser-extension-api-keys.title")}
              </p>
            </div>
            <p className="text-xs leading-[18px] font-base text-theme-text-secondary mt-2">
              {t("browser-extension-api-keys.description")}
            </p>
          </div>
          <div className="w-full justify-end flex">
            ...
            >
              ...
              {t("browser-extension-api-keys.generate-new")}
            </CTAButton>
          </div>
                ...
                <thead className="text-theme-text-secondary text-xs leading-[18px] font-bold uppercase border-theme-border border-b">
                  <tr>
                    <th scope="col" className="px-6 py-2 rounded-tl-lg">
                      {t("browser-extension-api-keys.table.connection-string")}
                    </th>
                    {isMultiUser && (
                      <th scope="col" className="px-6 py-2">
                        {t("browser-extension-api-keys.table.created-by")}
                      </th>
                    )}
                    <th scope="col" className="px-6 py-2">
                      {t("browser-extension-api-keys.table.created-at")}
                    </th>
                    <th scope="col" className="px-6 py-2 rounded-tr-lg">
                      {t("browser-extension-api-keys.table.actions")}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {apiKeys.length === 0 ? (
                    <tr className="bg-transparent text-theme-text-secondary text-sm font-medium">
                      ...
                      >
                        {t("browser-extension-api-keys.no-keys")}
                      </td>
                    </tr>
                  ) : (
                    apiKeys.map((apiKey) => (
