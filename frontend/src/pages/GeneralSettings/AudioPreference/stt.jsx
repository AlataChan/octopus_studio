...
const UNAVAILABLE_STT_PROVIDER = {
  name: "Unavailable speech to text provider",
  value: "unavailable",
  logo: OctopusStudioIcon,
  options: () => null,
  description:
    "The saved speech to text provider is no longer available. Choose a supported provider to continue.",
};
...
export default function SpeechToTextProvider({ settings }) {
  ...
  const PROVIDERS = [
    {
      name: t("audio.stt.system-native"),
      value: "native",
      logo: OctopusStudioIcon,
      options: (settings) => <BrowserNative settings={settings} />,
      description: t("audio.stt.system-native-description"),
    },
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      showToast(t("audio.stt.save-error", { error }), "error");
    } else {
      showToast(t("audio.stt.save-success"), "success");
    }
    setSaving(false);
    setHasChanges(!!error);
  };
