...
const UNAVAILABLE_TTS_PROVIDER = {
  name: "Unavailable text to speech provider",
  value: "unavailable",
  logo: OctopusStudioIcon,
  options: () => null,
  description:
    "The saved text to speech provider is no longer available. Choose a supported provider to continue.",
};
...
export default function TextToSpeechProvider({ settings }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      showToast(t("audio.tts.save-error", { error }), "error");
    } else {
      showToast(t("audio.tts.save-success"), "success");
    }
    setSaving(false);
    setHasChanges(!!error);
  };
