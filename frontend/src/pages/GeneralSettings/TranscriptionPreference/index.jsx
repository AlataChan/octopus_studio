...
const UNAVAILABLE_TRANSCRIPTION_PROVIDER = {
  name: "Unavailable transcription provider",
  value: "unavailable",
  logo: OctopusStudioIcon,
  options: () => null,
  description:
    "The saved transcription provider is no longer available. Choose a supported provider to continue.",
};

export function resolveTranscriptionProvider(selectedProvider) {
  return (
    PROVIDERS.find((provider) => provider.value === selectedProvider) ??
    UNAVAILABLE_TRANSCRIPTION_PROVIDER
  );
}
...
export default function TranscriptionModelPreference() {
  ...
  useEffect(() => {
    if (!settings) return;
    setSelectedProvider(
      (current) => current ?? settings?.WhisperProvider ?? "local"
    );
  }, [settings]);
  ...
  const selectedProviderValue =
    selectedProvider ?? settings?.WhisperProvider ?? "local";
  const selectedProviderObject = resolveTranscriptionProvider(
    selectedProviderValue
  );
  const selectedProviderAvailable = PROVIDERS.some(
    (provider) => provider.value === selectedProviderValue
  );
            ...
            <div className="flex flex-col w-full px-1 md:pl-6 md:pr-[50px] py-16 md:py-6">
                      ...
                      <div className="flex-1 pl-4 pr-2 flex flex-col gap-y-1 overflow-y-auto white-scrollbar pb-4 max-h-[245px]">
                        {filteredProviders.map((provider) => (
                          <LLMItem
                            ...
                            checked={selectedProviderValue === provider.value}
              ...
              >
                {selectedProviderAvailable &&
                  selectedProviderObject.options(settings)}
              </div>
            </div>
          </form>
        </div>
