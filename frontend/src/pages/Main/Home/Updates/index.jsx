...
const NEWS_CACHE_CONFIG = {
  enabled:
    import.meta.env.VITE_ANNOUNCEMENTS_ENABLED === "true" &&
    !!import.meta.env.VITE_ANNOUNCEMENTS_URL?.trim(),
  announcementsDir: import.meta.env.VITE_ANNOUNCEMENTS_URL?.trim() || "",
  listPath: import.meta.env.VITE_ANNOUNCEMENTS_LIST_PATH || "/list.txt",
  cacheKey: "alata_announcements",
  legacyCacheKey: "anythingllm_announcements",
  ttl: 7 * 24 * 60 * 60 * 1000, // 1 week
};

const PLACEHOLDERS = [PlaceholderOne, PlaceholderTwo, PlaceholderThree];

function joinAnnouncementUrl(baseUrl, pathPart = "") {
  const base = String(baseUrl || "").replace(/\/+$/, "");
  const next = String(pathPart || "").replace(/^\/+/, "");
  if (!base) return "";
  if (!next) return base;
  return `${base}/${next}`;
}
...
export default function Updates() {
  if (!NEWS_CACHE_CONFIG.enabled) return null;
  return <EnabledUpdates />;
}

function EnabledUpdates() {
...
function getCachedNews() {
  return (
    getCachedNewsByKey(NEWS_CACHE_CONFIG.cacheKey) ||
    getCachedNewsByKey(NEWS_CACHE_CONFIG.legacyCacheKey)
  );
}

function getCachedNewsByKey(cacheKey) {
  try {
...
async function fetchRemoteNews() {
  ...
  try {
    const articlesUrl = joinAnnouncementUrl(
      NEWS_CACHE_CONFIG.announcementsDir,
      NEWS_CACHE_CONFIG.listPath
    );
    const latestArticleDateRef = await fetch(articlesUrl)
      .then((res) => {
        if (!res.ok)
          throw new Error(
            `${res.status} - Failed to fetch remote news from ${articlesUrl}`
          );
        return res.text();
      })
    ...
    const dataURL = joinAnnouncementUrl(
      NEWS_CACHE_CONFIG.announcementsDir,
      `${latestArticleDateRef}${latestArticleDateRef.endsWith(".json") ? "" : ".json"}`
    );
    /** @type {NewsItem[]|null} */
