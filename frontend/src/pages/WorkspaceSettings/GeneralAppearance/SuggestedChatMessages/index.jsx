  ...
  if (loading)
    return (
      <div className="flex flex-col">
        ...
        <div className="text-theme-text-primary text-opacity-60 text-sm font-medium mt-6">
          <PreLoader size="4" />
        </div>
      </div>
    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-white/60 text-xs mt-2 w-full justify-center max-w-[600px]">
        {suggestedMessages.map((suggestion, index) => (
          <div key={index} className="relative w-full">
            <button
              className="transition-all duration-300 absolute z-10 text-neutral-700 bg-white rounded-full hover:bg-zinc-600 hover:border-zinc-600 hover:text-theme-text-primary border-transparent border shadow-lg ml-2"
              style={{
                top: -8,
                left: 265,
              }}
            ...
            <button
              ...
              className={`text-left p-2.5 border rounded-xl w-full border-theme-border-medium bg-theme-settings-input-bg hover:bg-theme-sidebar-item-selected-gradient ${
