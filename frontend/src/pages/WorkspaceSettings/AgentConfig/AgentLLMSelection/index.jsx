...
import { EffectiveAgentModelHint } from "@/components/WorkspaceChat/ChatContainer/PromptInput/LLMSelector/effectiveAgentModel";
...
const UNAVAILABLE_AGENT_LLM = {
  name: "Unavailable agent LLM provider",
  value: "unavailable",
  logo: OctopusStudioIcon,
  options: () => <React.Fragment />,
  description:
    "The saved agent LLM provider is no longer available. Choose a supported provider to continue.",
  requiredConfig: [],
};
...
export default function AgentLLMSelection({
  ...
  const selectedLLMObject =
    LLMS.find((llm) => llm.value === selectedLLM) ?? UNAVAILABLE_AGENT_LLM;
  const selectedLLMAvailable = LLMS.some((llm) => llm.value === selectedLLM);
  const displayLLM =
    selectedLLMObject?.value === "none"
      ? {
          ...selectedLLMObject,
          name: t("agent.default_name"),
          description: t("agent.default_description"),
        }
      : selectedLLMObject;
  ...
  return (
    <div className="border-b border-white/40 pb-8">
      ...
      <p className="text-theme-text-primary text-opacity-60 text-xs font-medium pb-3">
        {t("agent.intro")}
      </p>
      ...
      {selectedLLM !== "none" && selectedLLMAvailable && (
