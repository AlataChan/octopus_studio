...
const UNAVAILABLE_WORKSPACE_LLM = {
  name: "Unavailable workspace LLM provider",
  value: "unavailable",
  logo: OctopusStudioIcon,
  options: () => <React.Fragment />,
  description:
    "The saved workspace LLM provider is no longer available. Choose a supported provider to continue.",
  requiredConfig: [],
};
...
export default function WorkspaceLLMSelection({
  ...
  const selectedLLMObject =
    LLMS.find((llm) => llm.value === selectedLLM) ?? UNAVAILABLE_WORKSPACE_LLM;
  const selectedLLMAvailable = LLMS.some((llm) => llm.value === selectedLLM);
  const displayLLM =
    selectedLLMObject?.value === "default"
      ? {
          ...selectedLLMObject,
          name: t("chat.llm.default_name"),
          description: t("chat.llm.default_description"),
        }
      : selectedLLMObject;
