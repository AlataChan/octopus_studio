    ...
    <div className="flex gap-x-[5px] w-full items-center h-[32px]">
      <div className="relative h-full w-full flex">
        <input
          ...
          className="border border-transparent w-full h-full rounded-lg bg-theme-sidebar-item-default pl-9 focus:pl-4 pr-1 placeholder:text-theme-settings-input-placeholder outline-none text-theme-text-primary focus:border-[var(--theme-accent-border-soft)] search-input peer text-sm"
        />
        <MagnifyingGlass
          size={14}
...
function SearchResultWrapper({ children }) {
  return (
    <div className="absolute right-0 top-[6.2%] w-full flex flex-col gap-y-[24px] h-auto rounded-lg border border-theme-sidebar-border bg-theme-bg-secondary p-[16px] shadow-2xl z-10 max-h-[calc(100%-24px)] overflow-y-scroll no-scroll light:bg-theme-bg-primary light:border-2 light:border-theme-modal-border">
      {children}
    </div>
  );
}
...
function SearchResultItem({ to, name, hint }) {
  ...
  return (
    <Link
      ...
      className="hover:bg-[var(--theme-button-sidebar-hover-bg)] transition-all duration-300 rounded-sm px-[8px] py-[2px]"
    >
  ...
  return (
    <>
      <button
        ...
        className="border-none flex items-center justify-center bg-primary-button rounded-lg p-[8px] hover:bg-[var(--theme-button-primary-hover)] transition-all duration-300 text-[var(--theme-button-primary-text)]"
      >
