...
export default function ThreadItem({
  ...
  const isCurrentPath = window.location.pathname === linkTo;
  const shouldDisableNavigation = isCurrentPath || ctrlPressed;
  const threadLinkProps = {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={`${
          isActive
            ? "border-l-2 border-b-2 border-theme-border-strong light:border-theme-sidebar-border z-[2]"
            : "border-l border-b border-theme-border-medium light:border-theme-sidebar-border z-[1]"
        ...
        <div
          ...
          className={`${
            idx <= activeIdx && !isActive
              ? "border-l-2 border-theme-border-strong light:border-theme-sidebar-border z-[2]"
              : "border-l border-theme-border-medium light:border-theme-sidebar-border z-[1]"
      ...
      />
      <div
        className={`flex w-full items-center justify-between pr-2 group relative ${isActive ? "bg-[var(--theme-sidebar-thread-selected)] border border-solid border-transparent" : "hover:bg-theme-sidebar-subitem-hover"} rounded-[4px]`}
      >
        {thread.deleted ? (
          <div className="w-full flex justify-between">
            ...
            {ctrlPressed && (
              ...
              >
                <ArrowCounterClockwise
                  className="text-theme-text-secondary hover:text-theme-text-primary light:text-theme-text-secondary hover:light:text-theme-text-primary"
                  size={18}
                />
              </button>
            )}
          ...
          <>
            {shouldDisableNavigation ? (
              <span {...threadLinkProps}>{threadName}</span>
            ) : (
              <Link to={linkTo} {...threadLinkProps}>
                {threadName}
              </Link>
            )}
          </>
        ...
        {!!thread.slug && !thread.deleted && (
          <div ref={optionsContainer} className="flex items-center">
            ...
            {ctrlPressed ? (
              ...
              >
                <X
                  className="text-theme-text-secondary light:text-theme-text-secondary hover:text-theme-text-primary hover:light:text-theme-text-primary"
                  weight="bold"
                  size={18}
                />
              </button>
...
function OptionsMenu({
  ...
  return (
    <div
      ref={menuRef}
      className="absolute w-fit z-[20] top-[25px] right-[10px] bg-theme-bg-primary light:bg-theme-bg-sidebar light:border-[1px] light:border-theme-sidebar-border rounded-lg p-1"
    >
      <button
        onClick={renameThread}
        type="button"
        className="w-full rounded-md flex items-center p-2 gap-x-2 hover:bg-theme-sidebar-item-hover text-theme-text-secondary light:text-theme-text-primary"
      >
      ...
      <button
        ...
        type="button"
        className="w-full rounded-md flex items-center p-2 gap-x-2 hover:bg-theme-button-danger-hover-bg text-theme-text-secondary light:text-theme-text-primary hover:text-theme-button-danger-text"
      >
        <Trash size={18} />
