    ...
    <div className="flex flex-col w-fit">
      <div className="flex flex-col w-60">
        ...
        <div className="flex items-center w-fit gap-x-4 mb-2">
          <select
            ...
            value={selectedVoice}
            className="border-none flex-shrink-0 bg-theme-settings-input-bg border-theme-border text-theme-text-primary text-sm rounded-lg block w-full p-2.5"
          >
      ...
      {!!voices.find((voice) => voice.is_stored) && (
        <button
          type="button"
          onClick={flushVoices}
          className="w-fit border-none hover:text-theme-text-primary hover:underline text-white/40 text-sm my-4"
        >
          Flush voice cache
        </button>
      )}
...
function DemoVoiceSample({ voiceId }) {
  ...
  return (
    <button
      ...
      disabled={loading}
      className="border-none text-theme-text-secondary flex items-center gap-x-1"
    >
      {speaking ? (
        <>
