...
export function DnDFileUploaderProvider({
  ...
  async function handlePastedAttachment(event) {
    ...
    for (const file of files) {
      const attachment = await buildQueuedAttachment(file);
      if (attachment) newAccepted.push(attachment);
    }
...
export default function DnDFileUploaderWrapper({ children }) {
  ...
  const { getRootProps, getInputProps, open } = useDropzone({
    onDrop,
    // 不使用 disabled，因为它会阻止程序化点击
    // 改为在 onDrop 中检查 ready 状态
    noClick: true,
    noKeyboard: true,
    onDragEnter: () => ready && setDragging(true),
    onDragLeave: () => setDragging(false),
  });

  // 暴露 open 函数供 AttachItem 使用
  useEffect(() => {
    // 将 open 函数和 ready 状态挂载到 window，供 AttachItem 调用
    window.__dndDropzoneOpen = ready ? open : null;
    return () => {
      delete window.__dndDropzoneOpen;
    };
  }, [open, ready]);

  return (
    <div
      className={`relative flex flex-col flex-1 min-h-0 w-full md:mt-0 mt-[40px] p-[1px]`}
      {...getRootProps()}
    >
      <div
        hidden={!dragging}
        className="absolute top-0 w-full h-full bg-dark-text/90 light:bg-[#C2E7FE]/90 rounded-2xl border-[4px] border-white z-modal"
      >
...
/**
 * Convert chat attachment types into Base64 strings for requests.
 * @param {File} file
