...
import { isSupportedVideoAttachmentFile } from "@/utils/chat/videoAttachments";
...
function AttachmentItem({ attachment }) {
  ...
  const {
    iconBgColor,
    Icon,
    iconColor = "text-theme-attachment-icon",
  } = displayFromFile(file);
  const isVideoAttachment = isSupportedVideoAttachmentFile(file);
    ...
    return (
      <>
        ...
        >
          ...
          {contentString && !isVideoAttachment ? (
            <img
          ...
          <div className="flex flex-col w-[125px]">
            ...
            <p className="text-theme-attachment-text-secondary text-[10px] leading-[14px] font-medium">
              {isVideoAttachment ? "Video attached!" : "Image attached!"}
            </p>
          </div>
        </div>
        <Tooltip
...
function displayFromFile(file) {
  ...
  switch (extension) {
    ...
    case "flv":
    case "3gp":
      return { iconBgColor: "bg-purple", Icon: FileVideo };
    default:
