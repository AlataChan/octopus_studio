...
function GeminiModelSelection({ apiKey, settings }) {
  ...
  const sortedOrganizations = Object.keys(groupedModels).sort((a, b) => {
    if (a === "Stable") return -1;
    if (b === "Stable") return 1;
    return a.localeCompare(b);
  });
  const flatModels = sortedOrganizations.flatMap(
    (organization) => groupedModels[organization] || []
  );
  const defaultModelPref =
    settings?.GeminiLLMModelPref &&
    flatModels.some((m) => m.id === settings.GeminiLLMModelPref)
      ? settings.GeminiLLMModelPref
      : flatModels[0]?.id || "";
