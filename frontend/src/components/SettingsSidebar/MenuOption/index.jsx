...
import { preloadSettingsRoute } from "@/utils/settingsRoutePreload";
...
export default function MenuOption({
  ...
  if (!isChild) {
    ...
    if (!hasChildren) {
      if (isRoleRestricted({ flex, roles, user })) return null;
    }
  ...
  useEffect(() => {
    if (hasChildren) {
      childOptions.forEach((child) => {
        ...
        preloadSettingsRoute(child.href);
      });
      return;
    }

    if (href) preloadSettingsRoute(href);
  }, [childOptions, flex, hasChildren, href, roles, user]);
...
function hasVisibleOptions(user = null, childOptions = []) {
  ...
  function isVisible({
    ...
    if (isRoleRestricted({ roles, user, flex })) return false;
    return true;
  }
...
function isRoleRestricted({ roles = [], user = null, flex = false }) {
  if (!Array.isArray(roles) || roles.length === 0) return false;

  // Single-user mode does not persist a role-bearing user object in local storage.
  // Keep admin/manager menus visible there and rely on route guards for access control.
  if (!user) return false;

  if (!flex && !roles.includes(user?.role)) return true;
  if (flex && !roles.includes(user?.role)) return true;
  return false;
}
