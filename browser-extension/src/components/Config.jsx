...
export default function Config({ status, onStatusChange }) {
  ...
  async function disconnectFromExtension() {
    ...
    onStatusChange();
    setSaveStatus("Successfully disconnected from Alata");
  ...
  const handleConnect = async () => {
    try {
      ...
      if (!online) {
        setSaveStatus(
          "Alata is currently offline. Please try again later."
        );
        return;
      }
      ...
      onStatusChange();
      setSaveStatus("Successfully connected to Alata");
      chrome.runtime.sendMessage({ action: "connectionUpdated" });
    } catch (error) {
