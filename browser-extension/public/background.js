const ContextMenuModel = {
  async create(workspaces) {
    ...
    if (workspaces && workspaces.length > 0) {
      ...
      chrome.contextMenus.create({
        id: "saveEntirePageToAlata",
        title: "Save entire page to Alata",
        contexts: ["page"],
      });
...
const ExtensionModel = {
  ...
  async saveToAlata(selectedText, pageTitle, pageUrl) {
    const { apiBase, apiKey } = await chrome.storage.sync.get([
      "apiBase",
      "apiKey",
    ]);
  ...
  async saveEntirePageToAlata(pageContent, pageTitle, pageUrl) {
    const { apiBase, apiKey } = await chrome.storage.sync.get([
      "apiBase",
      "apiKey",
    ]);
  ...
  async handleResponse(response, action) {
    if (response.status === 401 || response.status === 403) {
      ...
      this.showNotification(
        "success",
        "Successfully saved content to Alata."
      );
    }
  },
  ...
  showNotification(type, message) {
    ...
    setTimeout(() => {
      ...
      chrome.action.setTitle({ title: "Alata Extension" });
    }, 5000);
  },
};
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === "saveToAlata") {
    ExtensionModel.saveToAlata(info.selectionText, tab.title, tab.url);
    return;
  }
  ...
  if (info.menuItemId === "saveEntirePageToAlata") {
    getPageContent(tab.id)
      .then((content) => {
        ExtensionModel.saveEntirePageToAlata(content, tab.title, tab.url);
      })
      .catch((error) => {
