# Run Alata Studio in production without Docker

> [!WARNING]
> This method of deployment is to be used as a reference for your deployment.
> You are fully responsible for securing your deployment and data in this mode.
> **Any issues** experienced from bare-metal or non-containerized deployments will **not** be officially supported.

Here you can find the scripts and known working process to run **Alata Studio** outside of a Docker container.
...
2. `cd alata-studio` and run `yarn setup`. This will install all dependencies to run in production as well as debug the application.
...
Alata Studio is comprised of three main sections: `frontend`, `server`, and `collector`. When running in production you will be running `server` and `collector` as two different processes, with a build step for compilation of the frontend.
...
Alata Studio should now be running on `http://localhost:3001`!

## Updating Alata Studio

To update Alata Studio with future updates you can `git pull origin master` to pull in the latest code and then repeat steps 2 - 5 to deploy with all changes fully.
...
_note_ You should `pkill node` before running an update so that you are not running multiple Alata Studio processes on the same instance as this can cause conflicts.
...
cd $HOME/alata-studio/frontend && yarn && yarn build && cd $HOME/alata-studio
...
cd $HOME/alata-studio/collector && yarn

echo "Installing server dependencies & running migrations"
cd $HOME/alata-studio/server && yarn
cd $HOME/alata-studio/server && npx prisma migrate deploy --schema=./prisma/schema.prisma
cd $HOME/alata-studio/server && npx prisma generate
...
>
> Alata Studio is a derivative work based on the MIT-licensed project [AnythingLLM](https://github.com/Mintplex-Labs/anything-llm) by Mintplex Labs.  
> The bare-metal deployment process above follows the same overall architecture, adapted to this distribution. For upstream-specific support or instructions, please refer to the original AnythingLLM repository and documentation.
