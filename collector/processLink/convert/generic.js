...
const {
  ...
  hasPuppeteer,
  isLightweightMode,
} = require("../../utils/featureDetection");
const { scrapeWithJinaReader } = require("./jinaReader");
...
async function getPageContent({ link, captureAs = "text", headers = {} }) {
  ...
  if (canUseJina) {
    try {
      const content = await scrapeWithJinaReader(link);
      if (content) return content;
    } catch (error) {
      console.error(
        "getPageContent failed to be fetched by Jina Reader - falling back!",
        error
      );
    }
  }
  ...
  const canUsePuppeteer =
    scraperPreference === "puppeteer" ||
    (!canUseJina && (scraperPreference === "" || scraperPreference === "auto"));

  if (canUsePuppeteer) {
    try {
      return await scrapeWithPuppeteerCore({
        link,
        captureAs,
      ...
      console.error(
        "getPageContent failed to be fetched by puppeteer-core - falling back to fetch!",
        error
      );
    }
  }
...
async function scrapeWithPuppeteerCore({
  link,
  captureAs,
  headers = {},
  runtimeSettings,
  ...
  if (!hasPuppeteer()) {
    throw new Error(
      "Puppeteer is not installed. Install puppeteer-core (and a Chromium binary) or use WEB_SCRAPER=jina."
    );
  }

  let puppeteer;
  try {
    // Prefer puppeteer-core in Lite mode to avoid bundled Chromium downloads.
    ...
    throw new Error(
      `Failed to load puppeteer module. ${error?.message || String(error)}`
    );
  }
  ...
  try {
    ...
    await page.goto(link, { timeout: 180000, waitUntil: "networkidle2" });
