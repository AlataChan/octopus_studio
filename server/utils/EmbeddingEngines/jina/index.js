 ...
 * 2. API 调用 (通过 Jina AI API) - 需要 API Key
 *
 * 常用模型:
 * - jina-embeddings-v2-base-zh (中文, 768维)
 * - jina-embeddings-v2-base-en (英文, 768维)
 * - jina-embeddings-v2-small-en (英文轻量, 512维)
 */
class JinaEmbedder {
  constructor() {
    // 检查部署模式
    this.useOllama = process.env.JINA_USE_OLLAMA === "true";
    this.className = "JinaEmbedder";

    if (this.useOllama) {
      // 本地部署模式 (通过 Ollama)
      if (!process.env.EMBEDDING_BASE_PATH)
        throw new Error("No embedding base path was set for Jina Ollama mode.");
      if (!process.env.EMBEDDING_MODEL_PREF)
        throw new Error("No embedding model was set for Jina Ollama mode.");
      ...
      try {
        ...
        throw new Error(
          [
            "Jina embedder (Ollama mode) requires the optional dependency `ollama`.",
            "Install it with `yarn workspace anything-llm-server add ollama` (and set LIGHTWEIGHT_MODE=false to enable local engines), or use Jina API mode instead.",
            error?.message || error,
          ]
            .filter(Boolean)
            .join(" ")
      ...
      if (!process.env.JINA_API_KEY)
        throw new Error("No Jina AI API key was set.");

      this.apiKey = process.env.JINA_API_KEY;
      this.baseURL = process.env.JINA_BASE_URL || "https://api.jina.ai/v1";
      this.model =
        process.env.EMBEDDING_MODEL_PREF || "jina-embeddings-v2-base-en";
      this.maxConcurrentChunks = 100; // Jina API 支持批量处理
      this.embeddingMaxChunkLength = 8192; // Jina v2 支持最大 8192 tokens
  ...
  async embedChunks(textChunks = []) {
    if (!(await this.#isAlive())) {
      const service = this.useOllama ? "Ollama" : "Jina AI API";
  ...
  async #embedWithOllama(textChunks) {
    ...
    for (const chunk of textChunks) {
      try {
        ...
        if (!Array.isArray(embedding) || embedding.length === 0)
          throw new Error("Jina Ollama returned an empty embedding for chunk!");
    ...
    if (!!error) throw new Error(`Jina Ollama Failed to embed: ${error}`);
  ...
  async #embedWithAPI(textChunks) {
    ...
    if (!!error) throw new Error(`Jina AI API Failed to embed: ${error}`);
    return data.length > 0 ? data : null;
  }
}
