...
function getOllamaSdk() {
  try {
    ...
    throw new Error(
      [
        "Ollama embedding engine requires the optional dependency `ollama`.",
        "Install it with `yarn workspace anything-llm-server add ollama` (and set LIGHTWEIGHT_MODE=false to enable local engines), or use a cloud embedding provider instead.",
        error?.message || error,
      ]
        .filter(Boolean)
        .join(" ")
