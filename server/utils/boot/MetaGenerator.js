...
class MetaGenerator {
  name = "MetaGenerator";
  defaultTitle = "Octopus Studio | Where your AI team comes to work";
  ...
  #defaultImage() {
    return "https://alata.studio/favicon.png";
  }
  ...
  #defaultMeta() {
    return [
      ...
      {
        tag: "meta",
        props: { property: "og:url", content: "https://alata.studio" },
      },
      {
        tag: "meta",
        props: {
      ...
      {
        tag: "meta",
        props: { property: "twitter:url", content: "https://alata.studio" },
      },
      {
        tag: "meta",
        props: {
  ...
  async #fetchConfg() {
    ...
    // An uploaded square app icon (white-label) supersedes the legacy favicon URL.
    const appIcon = await SystemSettings.getValueOrFallback(
      { label: "app_icon" },
      null
    );

    // If nothing defined - assume defaults.
    if (customTitle === null && faviconURL === null && !appIcon) {
      this.#customConfig = this.#defaultMeta();
    } else {
      const config = [];
      if (appIcon) {
        const v = encodeURIComponent(appIcon);
        config.push({
          tag: "link",
          props: {
            rel: "icon",
            type: "image/png",
            href: `/system/app-icon/favicon.png?v=${v}`,
          },
        });
        config.push({
          tag: "link",
          props: {
            ...
            href: `/system/app-icon/apple-touch.png?v=${v}`,
          },
        });
      } else {
        config.push({
