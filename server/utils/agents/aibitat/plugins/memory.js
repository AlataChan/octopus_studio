        ...
        aibitat.function({
          ...
          search: async function (query = "") {
            try {
              ...
              // 【修复】将知识库来源存储到 aibitat 实例，供 chat-history 插件保存
              if (sources.length > 0) {
                if (!this.super._knowledgeSources) {
                  this.super._knowledgeSources = [];
                }
                // 格式化 sources 以匹配前端 Citations 组件期望的格式
                const formattedSources = sources.map((source) => ({
                  id: source.id || source.vectorId,
                  title: source.title || source.metadata?.title || "未知文档",
                ...
                this.super._knowledgeSources.push(...formattedSources);
              }
