...
const SessionEngine = require("./runtime/sessionEngine");
...
class EphemeralAgentHandler extends AgentHandler {
  ...
  isSessionEngineEnabled() {
    return process.env.USE_SESSION_ENGINE === "true";
  }
  ...
  async createAIbitat(
    ...
    this.aibitat = new AIbitat({
      ...
      handlerProps: {
        ...
        user,
        log: this.log,
        // Ephemeral agent invocations are also treated as auto/long-task mode.
  ...
  #buildAgentRoute() {
  ...
  #ensureSessionEngine() {
    if (!this.aibitat) return null;

    const route = this.#buildAgentRoute();
    const sessionId = this.#sessionId || this.#invocationUUID;
    ...
    if (!this.sessionEngine) {
      this.sessionEngine = new SessionEngine({
        ...
        agentConfig: {
          ...
          model: this.model,
          maxTurns: this.aibitat.maxRounds,
        },
        tools: this.aibitat.functions,
        transcript: this.transcriptStore,
        eventLog: this.aibitat.getEventLog?.() || null,
    ...
    this.sessionEngine.workspaceId = this.#workspace?.id || null;
    this.sessionEngine.agentConfig = {
      ...this.sessionEngine.agentConfig,
      provider: this.provider,
      model: this.model,
      maxTurns: this.aibitat.maxRounds,
    };
    this.sessionEngine.tools = this.aibitat.functions;
    this.sessionEngine.eventLog = this.aibitat.getEventLog?.() || null;
    this.sessionEngine.aibitat = this.aibitat;
    this.sessionEngine.route = {
      from: route.from,
      to: route.to,
    };
    if (!this.sessionEngine.transcript) {
      this.sessionEngine.transcript = this.transcriptStore;
    }
  ...
  createSessionEngine() {
    return this.#ensureSessionEngine();
  }

  startAgentCluster() {
    if (this.isSessionEngineEnabled()) {
      const engine = this.#ensureSessionEngine();
      if (!engine) {
        throw new Error(
          "SessionEngine requires an initialized AIbitat instance."
        );
      }

      return (async () => {
        for await (const _event of engine.submitMessage(this.#prompt)) {
          // Phase 4 only needs the runtime layering; event fan-out remains owned
          // by the existing HTTP/WebSocket plugins attached to AIbitat.
        }
        return this.aibitat;
      })();
    }

    return this.aibitat.start(this.#buildAgentRoute());
  }
