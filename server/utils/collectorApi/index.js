...
const { resolveServiceHost } = require("../bindHost");
...
class CollectorApi {
  constructor() {
    ...
    const host = resolveServiceHost(process.env, "COLLECTOR_HOST") || "0.0.0.0";
    this.endpoint = `http://${host}:${process.env.COLLECTOR_PORT || defaultPort}`;
  }
  ...
  async #parseJsonResponse(res) {
    ...
    if (!text || text.trim() === "OK") {
      throw new Error("Collector returned empty or invalid response");
    }
    ...
    try {
      ...
      this.log(`Failed to parse JSON response: ${text.substring(0, 100)}`);
  ...
  /**
   * 获取 OCR 引擎状态
   * @returns {Promise<Object>} - OCR 引擎状态
   */
  async getOCRStatus() {
    return await fetch(`${this.endpoint}/ocr/status`)
      .then((res) => {
        if (!res.ok) throw new Error("Failed to get OCR status");
        return res.json();
      })
      .catch((e) => {
        this.log(e.message);
  ...
  async setupPaddleOCR() {
    return await fetch(`${this.endpoint}/ocr/paddle/setup`, {
      method: "POST",
   ...
   */
  async getOCRMetrics() {
    return await fetch(`${this.endpoint}/ocr/metrics`)
      .then((res) => {
        if (!res.ok) throw new Error("Failed to get OCR metrics");
        return res.json();
      })
      .catch((e) => {
        this.log(e.message);
   ...
   */
  async clearOCRCache() {
    return await fetch(`${this.endpoint}/ocr/cache/clear`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    })
      .then((res) => res.json())
