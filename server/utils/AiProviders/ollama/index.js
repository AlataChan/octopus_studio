...
function getOllamaSdk() {
  try {
    return require("ollama");
  } catch (error) {
    throw new Error(
      [
        "Ollama provider requires the optional dependency `ollama`.",
        "Install it with `yarn workspace anything-llm-server add ollama` (and set LIGHTWEIGHT_MODE=false to enable local providers), or use a cloud provider instead.",
        error?.message || error,
      ]
        .filter(Boolean)
        .join(" ")
...
class OllamaAILLM {
  ...
  #errorHandler(e) {
    switch (e.message) {
      case "fetch failed":
        throw new Error(
          "Your Ollama instance could not be reached or is not responding. Please make sure it is running the API server and your connection information is correct in Alata."
        );
      default:
        return e;
    }
