...
const { PromptCacheManager, CACHE_STRATEGIES } = require("../promptCache");
...
class DeepSeekLLM {
  constructor(embedder = null, modelPreference = null) {
    ...
    // Context Caching 配置 (DeepSeek V3 自动支持前缀缓存)
    this.cacheManager = new PromptCacheManager(CACHE_STRATEGIES.DEEPSEEK);
    this.cachingEnabled = process.env.DEEPSEEK_CONTEXT_CACHING !== "false";

    this.log(
      `Initialized ${this.model} with context window ${this.promptWindowLimit()}`
    );
    if (this.cachingEnabled) {
      this.log(`Context Caching enabled`);
    }
  }
  ...
  async getChatCompletion(messages = null, { temperature = 0.7 }) {
    ...
    // 更新缓存统计 (DeepSeek V3 返回 prompt_cache_hit_tokens)
    if (this.cachingEnabled && result.output.usage) {
      this.cacheManager.updateStats(
        CACHE_STRATEGIES.DEEPSEEK,
        result.output.usage
      );
    }
    ...
    return {
      ...
      metrics: {
        ...
        prompt_cache_hit_tokens:
          result.output.usage.prompt_cache_hit_tokens || 0,
        prompt_cache_miss_tokens:
          result.output.usage.prompt_cache_miss_tokens || 0,
      },
    };
  }
