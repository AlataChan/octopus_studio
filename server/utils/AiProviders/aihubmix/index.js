...
class AiHubMixLLM {
  constructor(embedder = null, modelPreference = null) {
    ...
    if (!process.env.AIHUBMIX_API_KEY)
      throw new Error("AiHubMix must have a valid API key set.");

    this.className = "AiHubMixLLM";
    this.basePath = parseOpenAiCompatibleBasePath(
    ...
    this.model = modelPreference ?? process.env.AIHUBMIX_MODEL_PREF ?? null;
    this.maxTokens = process.env.AIHUBMIX_MAX_TOKENS
      ? toValidNumber(process.env.AIHUBMIX_MAX_TOKENS, 1024)
      : 1024;

    if (!this.model) throw new Error("AiHubMix must have a valid model set.");
  ...
  static promptWindowLimit(_modelName) {
    const limit = process.env.AIHUBMIX_MODEL_TOKEN_LIMIT || 4096;
