...
function isValidUrl(str) {
  ...
  try {
    ...
    // 确保有有效的主机名（不是空的且包含点号，除非是 localhost）
    return (
      url.hostname &&
 ...
 * @returns {Promise<string>} Scraped content (empty string if URL is invalid)
 */
async function executeWebScraping(config, context) {
  ...
  // 条件跳过：如果 URL 为空或不是有效的 URL 格式，跳过此步骤
  if (!url || !isValidUrl(url)) {
    const reason = !url
      ? "URL is empty"
      : `"${url.substring(0, 50)}..." is not a valid URL`;
    introspect(
      `[Web Scraping] Skipping: ${reason}. Continuing with empty content.`
    );
    logger(`[Web Scraping] Skipped - ${reason}`);
    return ""; // 返回空字符串，让后续步骤继续执行
  }
