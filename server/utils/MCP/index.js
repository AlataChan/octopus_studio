...
const ToolDescriptor = require("../agents/runtime/toolDescriptor");
...
class MCPCompatibilityLayer extends MCPHypervisor {
  ...
  /**
   * Convert an MCP server's tools into runtime ToolDescriptor objects.
   * The legacy plugin-based registration path remains unchanged.
   *
   * @param {string} name
   * @returns {Promise<ToolDescriptor[]|null>}
   */
  async convertServerToolsToDescriptors(name) {
    const mcp = this.mcps[name];
    if (!mcp) return null;
        ...
        new ToolDescriptor({
          ...
          handler: async (args = {}) => {
            try {
              ...
              return `Tool ${name}:${tool.name} failed: ${error?.message || "Unknown error"}`;
            }
          },
          isConcurrencySafe: false,
          isReadOnly: !!tool.annotations?.readOnlyHint,
          isDestructive: !!tool.annotations?.destructiveHint,
          riskLevel: tool.annotations?.destructiveHint
            ? RiskLevel.EXECUTE
            : RiskLevel.EXTERNAL,
          source: "mcp",
          mcpInfo: {
