...
async function fileData(filePath = null) {
  ...
  // 使用异步方法检查文件存在性和读取文件
  try {
    await fsPromises.access(fullFilePath, fs.constants.R_OK);
  } catch {
    return null;
  }
...
async function viewLocalFiles() {
  ...
  for (const file of files) {
    ...
    if (isFolder) {
      ...
      // 使用异步方法读取子目录
      const subfiles = await fsPromises.readdir(folderPath);
      const filenames = {};
      const filePromises = [];
...
async function getDocumentsByFolder(folderName = "") {
  ...
  if (!isWithin(documentsPath, folderPath)) {
    return {
      folder: folderName,
      documents: [],
      code: 404,
...
async function cachedVectorInformation(filename = null, checkOnly = false) {
  ...
  // 使用异步方法读取文件
  const rawData = await fsPromises.readFile(file, "utf8");
...
async function storeVectorResult(vectorData = [], filename = null) {
  ...
  // 使用异步方法创建目录和写入文件
  try {
    await fsPromises.access(vectorCachePath);
  } catch {
    await fsPromises.mkdir(vectorCachePath, { recursive: true });
  }

  const digest = uuidv5(filename, uuidv5.URL);
  const writeTo = path.resolve(vectorCachePath, `${digest}.json`);
  await fsPromises.writeFile(writeTo, JSON.stringify(vectorData), "utf8");
  return;
}
...
async function findDocumentInDocuments(documentName = null) {
  ...
  // 使用异步方法读取目录
  const folders = await fsPromises.readdir(documentsPath);
  ...
  for (const folder of folders) {
    ...
    // 使用异步方法检查文件存在性和读取文件
    try {
      await fsPromises.access(targetFileLocation, fs.constants.R_OK);
      const fileDataContent = await fsPromises.readFile(
        targetFileLocation,
        "utf8"
      );
      const cachefilename = `${folder}/${targetFilename}`;
      const { pageContent: _pageContent, ...metadata } =
        JSON.parse(fileDataContent);
      return {
        name: targetFilename,
        type: "file",
        ...metadata,
...
async function fileToPickerData({
  ...
  if (fileStats.size < FILE_READ_SIZE_THRESHOLD) {
    // 使用异步方法读取文件
    const rawData = await fsPromises.readFile(pathToFile, "utf8");
    try {
