...
async function chatSync({
  ...
  const filledSources = fillSourceWindow({
    ...
    searchResults: hybridResults.sources,
    history: rawHistory,
    filterIdentifiers: pinnedDocIdentifiers,
  });
  ...
  sources = [...sources, ...hybridResults.sources];

  // 【Phase 0】注入对话摘要上下文
  let summaryInjected = false;
  const summaryContext = getConversationSummaryContext(thread);
  if (summaryContext) {
    contextTexts.unshift(summaryContext);
    summaryInjected = true;
    console.log("[API] Summary injected for thread:", thread?.id);
  }

  // 【Phase 0】添加图谱上下文检索
  let graphContextUsed = false;
  ...
  const { chat } = await WorkspaceChats.new({
    ...
    response: {
      ...
      metadata: {
        graphContextUsed,
        summaryInjected,
        hybridRetrievalApplied: hybridResults.hybridApplied,
...
async function streamChat({
  ...
  // 【Phase 0】应用混合检索重排序
  const hybridResultsStream = applyHybridRetrieval({
    sources: vectorSearchResults.sources,
    contextTexts: vectorSearchResults.contextTexts,
    enabled: true,
  });
  ...
  const filledSources = fillSourceWindow({
    nDocs: workspace?.topN || 4,
    searchResults: hybridResultsStream.sources,
    history: rawHistory,
    filterIdentifiers: pinnedDocIdentifiers,
  });
  ...
  // TLDR; reduces GitHub issues for "LLM citing document that has no answer in it" while keep answers highly accurate.
  contextTexts = [...contextTexts, ...filledSources.contextTexts];
  sources = [...sources, ...hybridResultsStream.sources];

  // 【Phase 0】注入对话摘要上下文
  let summaryInjectedStream = false;
  const summaryContextStream = getConversationSummaryContext(thread);
  if (summaryContextStream) {
    contextTexts.unshift(summaryContextStream);
    summaryInjectedStream = true;
    console.log("[API Stream] Summary injected for thread:", thread?.id);
  }

  // 【Phase 0】添加图谱上下文检索
  let graphContextUsedStream = false;
  let graphContextSummaryStream = null;
  if (embeddingsCount !== 0) {
    const graphResult = await getGraphContextForChat({
      workspaceId: workspace.id,
      query: message,
    ...
    if (graphResult?.summary) {
      graphContextSummaryStream = graphResult.summary;
      contextTexts.push(graphContextSummaryStream);
      sources.push({
        text: `知识图谱上下文 (${graphResult.nodeCount} 个节点, ${graphResult.edgeCount} 条关系)`,
        title: "知识图谱",
        type: "graph",
      ...
      graphContextUsedStream = true;
    }
  }

  const kbRetrievalStream = await applyOctopusKbRetrieval({
    workspace,
    query: message,
    contextTexts,
    sources,
    graphSummary: graphContextSummaryStream,
  });
  contextTexts = kbRetrievalStream.contextTexts;
  sources = kbRetrievalStream.sources;
  ...
  if (completeText?.length > 0) {
    const { chat } = await WorkspaceChats.new({
      ...
      response: {
        ...
        attachments,
        metadata: {
          graphContextUsed: graphContextUsedStream,
          summaryInjected: summaryInjectedStream,
          hybridRetrievalApplied: hybridResultsStream.hybridApplied,
          octopusKbRetrieval: kbRetrievalStream.metadata,
        },
      },
      threadId: thread?.id || null,
      apiSessionId: sessionId,
