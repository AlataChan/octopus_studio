...
const { requiresSingleUserAuth } = require("../utils/authRuntime");
const { isDefaultLogoFilename } = require("../utils/files/defaultLogos");

const MASKED_SECRET = "*".repeat(20);
const DEFAULT_VIDEO_UNDERSTANDING_PROVIDER = "moonshot";
const DEFAULT_VIDEO_UNDERSTANDING_BASE_URL = "https://api.moonshot.ai/v1";
const DEFAULT_VIDEO_UNDERSTANDING_MODEL = "kimi-k2.6";
...
const SystemSettings = {
  ...
  supportedFields: [
    ...
    // Skill Hub settings
    "skill_hub_registries",

    // Hub settings
    "hub_api_key",

    // Molt integration settings
    "MOLT_BASE_URL",
    "MOLT_API_TOKEN_FILE",
    "MOLT_TOKEN_FILE",
    "MOLT_API_TOKEN",
    ...
    // Octopus KB integration settings
    "OCTOPUS_KB_ENABLED",
    "OCTOPUS_KB_COMMAND",
    "OCTOPUS_KB_ARGS",
    "OCTOPUS_KB_VAULT_ROOT",
    "OCTOPUS_KB_CURATION_ENABLED",
    "OCTOPUS_KB_CURATION_MAX_FILES",
    "OCTOPUS_KB_CURATION_MAX_BYTES",
    "OCTOPUS_KB_MEMORY_ENABLED",

    // Work-agent desktop/web runtime settings
    "ALATA_WORK_AGENT_PROVIDER",
    "ALATA_CODE_EXECUTION_ROOT",
  ...
  validations: {
    ...
    // Base id (uuid) of the uploaded square app icon. null => use default favicon.
    app_icon: (baseId) => {
      try {
    ...
    skill_hub_registries: (update) => {
      try {
        const parsed = safeJsonParse(update, []);
        ...
        const cleaned = list
          ...
          .map((r) => {
            ...
            const repo = r.repo ? String(r.repo) : null;
            const branch = r.branch ? String(r.branch) : null;
            const filePath = r.path ? String(r.path) : null;
            const priority = Number.isFinite(Number(r.priority))
              ? Number(r.priority)
              : null;
            const sourceType = r.sourceType ? String(r.sourceType) : null;

            // Must have either explicit url or owner+repo.
    ...
    MOLT_BASE_URL: (url) => {
      ...
      try {
        ...
        throw new Error("MOLT_BASE_URL must be a valid http(s) URL.");
      }

      if (!["http:", "https:"].includes(parsed.protocol)) {
        throw new Error("MOLT_BASE_URL must use http or https.");
      }

      return parsed.toString().replace(/\/+$/, "");
    },
    MOLT_API_TOKEN_FILE: (filePath) => {
      if (!filePath) return null;
      return String(filePath);
    },
    MOLT_TOKEN_FILE: (filePath) => {
      if (!filePath) return null;
      return String(filePath);
    },
    MOLT_API_TOKEN: (apiToken) => {
    ...
    MOLT_ENABLED: (update) => {
      ...
      if (!["true", "false"].includes(update)) return "false";
      return String(update);
    },
    billing_enabled: (update) => {
      if (typeof update === "boolean")
        return update === true ? "true" : "false";
      if (!["true", "false"].includes(update)) return "false";
      return String(update);
    },
    video_understanding_enabled: (update) => {
      if (typeof update === "boolean")
        return update === true ? "true" : "false";
      return String(update).trim().toLowerCase() === "true" ? "true" : "false";
    },
    video_understanding_provider: (provider) => {
      const value = String(provider || DEFAULT_VIDEO_UNDERSTANDING_PROVIDER)
        .trim()
        .toLowerCase();
      return ["moonshot"].includes(value)
        ? value
        : DEFAULT_VIDEO_UNDERSTANDING_PROVIDER;
    },
    video_understanding_base_url: (url) => {
      const value = String(url || "").trim();
      if (!value) return null;
      ...
      try {
        ...
        throw new Error(
          "video_understanding_base_url must be a valid http(s) URL."
        );
      }

      if (!["http:", "https:"].includes(parsed.protocol)) {
        throw new Error("video_understanding_base_url must use http or https.");
      }

      return parsed.toString().replace(/\/+$/, "");
    },
    video_understanding_model: (model) => {
      const value = String(model || "").trim();
      return value || DEFAULT_VIDEO_UNDERSTANDING_MODEL;
    },
    video_understanding_api_key: async (apiKey) => {
      const value = String(apiKey || "").trim();
      if (!value) return null;
      if (isMaskedSecret(value)) {
        return (
          (await SystemSettings.get({ label: "video_understanding_api_key" }))
            ?.value || null
        );
      }
      return value;
    },
    SEED_GSTACK_ASSISTANTS: (update) => {
      if (typeof update === "boolean")
        return update === true ? "true" : "false";
      return String(update).trim().toLowerCase() === "true" ? "true" : "false";
    },
    ALATA_WORK_AGENT_PROVIDER: (update) => {
      const provider = String(update || "")
        .trim()
        .toLowerCase();
      ...
      if (
        ...
        throw new Error("Invalid work-agent provider.");
      }
      return provider;
    },
    ALATA_CODE_EXECUTION_ROOT: (update) => {
      const root = String(update || "").trim();
      if (!root) return null;
      return path.resolve(root);
    },
    OCTOPUS_KB_ENABLED: (update) => {
      if (typeof update === "boolean")
        return update === true ? "true" : "false";
      return String(update).trim().toLowerCase() === "true" ? "true" : "false";
    },
    OCTOPUS_KB_MEMORY_ENABLED: (update) => {
      if (typeof update === "boolean")
        return update === true ? "true" : "false";
      return String(update).trim().toLowerCase() === "true" ? "true" : "false";
    },
    OCTOPUS_KB_COMMAND: (update) => {
      const command = String(update || "").trim();
    ...
    OCTOPUS_KB_ARGS: (update) => {
      if (Array.isArray(update)) return JSON.stringify(update.map(String));
      const raw = String(update || "").trim();
      if (!raw) return "[]";
    ...
    OCTOPUS_KB_VAULT_ROOT: (update) => {
      const root = String(update || "").trim();
      if (!root) return null;
      return path.resolve(root);
    },
    // Knowledge Graph validations
    kg_guided_retrieval_enabled: (update) => {
      if (typeof update === "boolean")
        return update === true ? "true" : "false";
      if (!["true", "false"].includes(update)) return "false";
      return String(update);
    },
    kg_entity_extraction_enabled: (update) => {
      if (typeof update === "boolean")
        return update === true ? "true" : "false";
      if (!["true", "false"].includes(update)) return "false";
      return String(update);
    },
    kg_similarity_edges_enabled: (update) => {
      if (typeof update === "boolean")
        return update === true ? "true" : "false";
      if (!["true", "false"].includes(update)) return "false";
      return String(update);
    },
    kg_path_finder_enabled: (update) => {
      if (typeof update === "boolean")
        return update === true ? "true" : "false";
      if (!["true", "false"].includes(update)) return "false";
      return String(update);
    },
    kg_search_timeout_ms: (update) => {
      try {
    ...
    kg_circuit_breaker_threshold: (update) => {
      try {
        const value = parseInt(update, 10);
  ...
  currentSettings: async function () {
    ...
    const multiUserMode = await this.isMultiUserMode();
    return {
      // --------------------------------------------------------
      // General Settings
      // --------------------------------------------------------
      ...
      FdeServiceConfigured: Boolean(process.env.FDE_SERVICE_URL),
      ...
      VideoUnderstandingEnabled: await this.videoUnderstandingEnabled(),
      VideoUnderstanding: await this.videoUnderstandingSettings(),
    };
  },
  ...
  currentLogoFilename: async function () {
    try {
      ...
      const filename = setting?.value || null;
      return isDefaultLogoFilename(filename) ? null : filename;
    } catch (error) {
      console.error(error.message);
      return null;
    }
  ...
  llmPreferenceKeys: function () {
    return {
      ...
      MoonshotAiBaseUrl:
        ...
        DEFAULT_VIDEO_UNDERSTANDING_BASE_URL,
      MoonshotAiModelPref:
      ...
      // AiHubMix Keys (OpenAI compatible)
      AiHubMixBasePath: process.env.AIHUBMIX_BASE_PATH,
      AiHubMixApiKey: !!process.env.AIHUBMIX_API_KEY,
      AiHubMixModelPref: process.env.AIHUBMIX_MODEL_PREF,
      AiHubMixTokenLimit: process.env.AIHUBMIX_MODEL_TOKEN_LIMIT,
      AiHubMixMaxTokens: process.env.AIHUBMIX_MAX_TOKENS,
   ...
   */
  isBillingEnabled: async function () {
    try {
      const setting = await this.get({ label: "billing_enabled" });
      return setting?.value === "true";
    } catch (error) {
      console.error(error.message);
      return false;
  ...
  videoUnderstandingEnabled: async function () {
    try {
      const setting = await this.get({ label: "video_understanding_enabled" });
      return setting?.value === "true";
    } catch (error) {
      console.error(error.message);
      return false;
  ...
  videoUnderstandingSettings: async function ({ maskApiKey = true } = {}) {
    const enabled = await this.videoUnderstandingEnabled();
    const provider =
      (await this.getValueOrFallback(
        { label: "video_understanding_provider" },
        null
      )) ||
      process.env.VIDEO_UNDERSTANDING_PROVIDER ||
      DEFAULT_VIDEO_UNDERSTANDING_PROVIDER;
    const baseUrl =
      (await this.getValueOrFallback(
        { label: "video_understanding_base_url" },
        null
      )) ||
      process.env.MOONSHOT_AI_BASE_URL ||
      DEFAULT_VIDEO_UNDERSTANDING_BASE_URL;
    const model =
      (await this.getValueOrFallback(
        { label: "video_understanding_model" },
        null
      )) ||
    ...
    return {
      enabled,
      provider: String(provider || DEFAULT_VIDEO_UNDERSTANDING_PROVIDER)
        .trim()
        .toLowerCase(),
      baseUrl,
      model,
      apiKey: maskApiKey ? maskSecret(apiKey) : apiKey,
    };
  },
