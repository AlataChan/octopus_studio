...
const { WorkspaceAssistant } = require("./workspaceAssistant");
...
const Workspace = {
  ...
  validations: {
    ...
    trajectoryMemoryDisabled: (value) => {
      if (value === null || value === undefined) return false;
      if (typeof value === "boolean") return value;
      if (typeof value === "string") {
        return value.toLowerCase() === "true" || value === "1";
  ...
  new: async function (name = null, creatorId = null, additionalFields = {}) {
    ...
    try {
      ...
      // 自动安装默认助手到新创建的 Workspace
      await this._installDefaultAssistants(workspace.id);
  ...
  /**
   * Install default assistants to a workspace
   * @private
   * @param {number} workspaceId - The ID of the workspace
   */
  _installDefaultAssistants: async function (workspaceId) {
    try {
      const defaultTemplates = await AssistantTemplate.getDefaultTemplates();
      if (!defaultTemplates || defaultTemplates.length === 0) {
        console.log("[Workspace] No default assistant templates found");
        return;
      }

      console.log(
        `[Workspace] Installing ${defaultTemplates.length} default assistants to workspace ${workspaceId}`
      );
      ...
      for (const template of defaultTemplates) {
        const { assistant, message } = await WorkspaceAssistant.install(
          workspaceId,
          template.id,
          null, // instanceName - 使用模板默认名称
          null, // customConfig
          "default" // source 标记为默认
        );

        if (assistant) {
          console.log(
            `[Workspace] Installed default assistant: ${template.name}`
          );
        } else {
          console.warn(
            `[Workspace] Failed to install default assistant ${template.name}: ${message}`
          );
        }
      }
    } catch (error) {
      console.error("[Workspace] Error installing default assistants:", error);
      // 不阻塞 Workspace 创建流程
    }
  },
  ...
  update: async function (id = null, updates = {}) {
    ...
    let previousTierRoutingOptOut = null;
    let previousTrajectoryMemoryDisabled = null;
    if (
      ...
      previousTierRoutingOptOut = await prisma.workspaces.findFirst({
        where: { id: Number(id) },
        select: { id: true, disableTierRouting: true },
      });
    }
    if (
      Object.prototype.hasOwnProperty.call(
      ...
      previousTrajectoryMemoryDisabled = await prisma.workspaces.findFirst({
        where: { id: Number(id) },
    ...
    let result;
    let trajectoryAuditWrittenInTransaction = false;
    if (validatedUpdates.trajectoryMemoryDisabled === true) {
      const {
        listTrajectoryNamespacesForWorkspace,
        dropTrajectoryNamespaces,
      } = require("../utils/agents/trajectoryMemory");
      const namespaces = await listTrajectoryNamespacesForWorkspace(Number(id));

      try {
        const workspace = await prisma.$transaction(async (tx) => {
          const updated = await tx.workspaces.update({
            where: { id: Number(id) },
            data: validatedUpdates,
          });
          await tx.agent_trajectories.deleteMany({
          ...
          await tx.event_logs.create({
            data: {
              ...
              metadata: JSON.stringify({
                ...
                old:
                  previousTrajectoryMemoryDisabled?.trajectoryMemoryDisabled ??
                  false,
                new: true,
                purged: true,
              }),
        ...
        trajectoryAuditWrittenInTransaction = true;
        result = { workspace, message: null };
      } catch (error) {
        console.error(error.message);
        result = { workspace: null, message: error.message };
      }
      ...
      if (result.workspace) {
        try {
          ...
          console.warn(
            "[Workspace] trajectory namespace cleanup skipped:",
            error.message
          );
        }
      }
    } else {
      result = await this._update(id, validatedUpdates);
    }

    if (
      result.workspace &&
      previousTierRoutingOptOut &&
      previousTierRoutingOptOut.disableTierRouting !==
        validatedUpdates.disableTierRouting
    ) {
      await EventLogs.logEvent(
        "workspace_tier_routing_optout_changed",
        {
          workspaceId: Number(id),
          old: previousTierRoutingOptOut.disableTierRouting,
          new: validatedUpdates.disableTierRouting,
        },
        null
      );
    }
    if (
      result.workspace &&
      previousTrajectoryMemoryDisabled &&
      !trajectoryAuditWrittenInTransaction
    ) {
      await EventLogs.logEvent(
        "workspace_trajectory_memory_disabled_changed",
        {
          workspaceId: Number(id),
          old: previousTrajectoryMemoryDisabled.trajectoryMemoryDisabled,
          new: validatedUpdates.trajectoryMemoryDisabled,
          purged: false,
        },
        null
      );
  ...
  delete: async function (clause = {}) {
    try {
      ...
      const workspaceId = workspace?.id ?? clause?.id;
      if (workspaceId) {
        const {
          dropTrajectoryNamespacesForWorkspace,
        } = require("../utils/agents/trajectoryMemory");
        try {
          await dropTrajectoryNamespacesForWorkspace(Number(workspaceId));
        } catch (error) {
          console.warn(
            "[Workspace] trajectory namespace cleanup skipped:",
            error.message
  ...
  _deleteOctopusKbMemoryPages: async function (clause = {}) {
    try {
      ...
      await deleteWorkspaceMemoryPages(workspace.slug);
    } catch (error) {
      console.warn("[Workspace] octopus-kb memory cleanup skipped:", error.message);
    }
  },
  ...
  whereWithUsers: async function (clause = {}, limit = null, orderBy = null) {
    try {
      ...
      // 批量获取所有工作区的用户关系 - 只需 2 次数据库查询而非 N+1 次
      const workspaceIds = workspaces.map((ws) => Number(ws.id));
      const allWorkspaceUsers = await WorkspaceUser.where({
        workspace_id: { in: workspaceIds },
      });

      // 按 workspace_id 分组
      const workspaceUserMap = {};
      for (const rel of allWorkspaceUsers) {
        if (!workspaceUserMap[rel.workspace_id]) {
          workspaceUserMap[rel.workspace_id] = [];
        }
        workspaceUserMap[rel.workspace_id].push(rel.user_id);
      }

      // 设置每个工作区的用户 ID 列表
      for (const workspace of workspaces) {
        workspace.userIds = workspaceUserMap[Number(workspace.id)] || [];
      }
