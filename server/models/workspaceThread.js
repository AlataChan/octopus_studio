...
const WorkspaceThread = {
  ...
  delete: async function (clause = {}) {
    try {
      ...
      if (threads.length > 0) {
        const { ExternalThreadState } = require("./externalThreadState");
        await this._deleteOctopusKbMemoryPages(threads);
        await Promise.all(
          threads.map((thread) =>
            ExternalThreadState.delete({
              ...
              console.warn(
                `[WorkspaceThread] Failed to delete external state for thread ${thread.id}:`,
                error.message
              );
              return false;
            })
  ...
  _deleteOctopusKbMemoryPages: async function (threads = []) {
    try {
      if (!threads.length || !prisma.workspaces?.findMany) return;
      const workspaceIds = Array.from(
        new Set(threads.map((thread) => thread.workspace_id).filter(Boolean))
      );
      ...
      for (const thread of threads) {
        const slug = slugsById.get(thread.workspace_id);
        if (!slug) continue;
        await deleteWorkspaceMemoryPages(slug, thread.id);
      }
    } catch (error) {
      console.warn("[WorkspaceThread] octopus-kb memory cleanup skipped:", error.message);
    }
  },
