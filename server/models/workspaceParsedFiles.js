...
function resolveDirectUploadLocation(location) {
  if (!location) return null;

  const directUploadsFolderName = path.basename(directUploadsPath);
  const cleaned = String(location).replace(/\\/g, "/").trim();
  ...
  const relativePath = normalizePath(relativeParts.join("/"));
  const resolved = path.resolve(directUploadsPath, relativePath);
  if (!isWithin(directUploadsPath, resolved)) return null;
  return resolved;
}

function uniqueEmbeddedFilenameFromLocation(location) {
  if (!location) return null;
  const directUploadsFolderName = path.basename(directUploadsPath);
  const cleaned = String(location).replace(/\\/g, "/").trim();
  if (!cleaned) return null;
  ...
  if (relativeParts.length === 1) return relativeParts[0];
  return `${relativeParts.slice(0, -1).join("-")}-${relativeParts.at(-1)}`;
}
...
const WorkspaceParsedFiles = {
  ...
  moveToDocumentsAndEmbed: async function (fileId, workspace) {
    try {
      ...
      const sourceFile = resolveDirectUploadLocation(location);
      ...
      const targetFilename =
        uniqueEmbeddedFilenameFromLocation(location) || path.basename(location);
      const targetPath = path.join(customDocsPath, targetFilename);
      ...
      const {
        ...
        workspace,
        [`custom-documents/${targetFilename}`],
        parsedFile.userId
      );
