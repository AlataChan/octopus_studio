...
function workspaceEndpoints(app) {
  ...
  app.get(
    ...
    async (request, response) => {
      try {
        ...
        // Support per-assistant filtering: pass assistantId when client sends it,
        // undefined when absent (backward-compat: returns all chats).
        const { assistantId: rawAssistantId } = request.query;
        const assistantId =
          "assistantId" in request.query
            ? rawAssistantId === "" || rawAssistantId === "null"
              ? null
              : rawAssistantId
            : undefined;
        ...
        const history = multiUserMode(response)
          ...
          : await WorkspaceChats.forWorkspace(
              workspace.id,
              null,
              null,
              assistantId
  ...
  app.post(
    ...
    async (request, response) => {
      try {
        ...
        // 同时记录到经验记忆（用于 Platform 模式分析）
        if (feedback !== null) {
          try {
            const { FeedbackCollector } = require("../utils/memory");
            await FeedbackCollector.recordFeedback({
              chatId: Number(chatId),
              feedback: feedback === true ? "positive" : "negative",
              userId: user?.id,
              workspaceId: workspace.id,
              platform: "internal",
              taskType: "qa",
            ...
            // 记录失败不影响主流程
            console.warn(
              "[ChatFeedback] Failed to record to experience memory:",
  ...
  // 用户评分 API（1-5 星）
  app.post(
    "/workspace/:slug/chat-user-rating/:chatId",
    [validatedRequest, flexUserRoleValid([ROLES.all]), validWorkspaceSlug],
    async (request, response) => {
      try {
        const { chatId } = request.params;
        ...
        if (rating !== null && (rating < 1 || rating > 5)) {
          response
            .status(400)
            .json({ success: false, error: "评分必须在 1-5 之间" });
          return;
        }
        ...
        if (!existingChat) {
          response.status(404).json({ success: false, error: "消息不存在" });
          return;
        }

        // 更新用户评分
        const result = await WorkspaceChats.updateUserRating(chatId, rating);

        // 记录事件日志
        if (rating !== null) {
          await EventLogs.logEvent("chat_user_rating", {
            chatId: Number(chatId),
            workspaceId: workspace.id,
            rating,
          });
        ...
        console.error("Error updating chat user rating:", error);
  ...
  /**
   * GET /workspace/:slug/assistants/:instanceId
   * Get a single workspace assistant by ID
   */
  app.get(
    "/workspace/:slug/assistants/:instanceId",
    [validatedRequest, validWorkspaceSlug],
    async (request, response) => {
      try {
        const { slug, instanceId } = request.params;
        const user = await userFromSession(request, response);
        ...
        const assistant = await WorkspaceAssistant.getById(instanceId);

        if (!assistant || assistant.workspace?.id !== workspace.id) {
          response.status(404).json({
            success: false,
            error: "助手不存在或不属于此工作区",
          });
  ...
  /**
   * PATCH /workspace/:slug/assistants/:instanceId
   * Update a workspace assistant (enable/disable, rename, custom config)
   */
  app.patch(
    "/workspace/:slug/assistants/:instanceId",
    [
    ...
    async (request, response) => {
      try {
        ...
        // 校验 knowledgeModeOverride
        if (
          updates.knowledgeModeOverride !== undefined &&
          updates.knowledgeModeOverride !== null
        ) {
          const validModes = ["workspace", "platform", "none"];
          if (!validModes.includes(updates.knowledgeModeOverride)) {
            response.status(400).json({
              success: false,
              error: `Invalid knowledgeModeOverride. Must be one of: ${validModes.join(", ")}`,
            });
            return;
          }
        }

        const { assistant, message } = await WorkspaceAssistant.update(
          instanceId,
          updates
        );
  ...
  /**
   * DELETE /workspace/:slug/assistants/:instanceId
  ...
  app.delete(
    ...
    async (request, response) => {
      try {
        ...
        const success = await WorkspaceAssistant.uninstall(instanceId);
        ...
        await EventLogs.logEvent(
          "workspace_assistant_uninstalled",
          {
            workspaceId: workspace.id,
            assistantId: instanceId,
          },
        ...
        console.error("Error uninstalling workspace assistant:", error);
        response.status(500).json({
          success: false,
          error: "服务器错误，请稍后重试",
        });
