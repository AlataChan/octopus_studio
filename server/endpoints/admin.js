...
function adminEndpoints(app) {
          ...
          switch (label) {
            ...
            case "meta_page_favicon":
              requestedSettings[label] =
                await SystemSettings.getValueOrFallback({ label }, null);
              break;
            case "video_understanding_enabled":
              requestedSettings[label] =
                (await SystemSettings.get({ label }))?.value || "false";
              break;
            case "video_understanding_provider":
            case "video_understanding_base_url":
            case "video_understanding_model":
            case "video_understanding_api_key": {
              const videoSettings =
                await SystemSettings.videoUnderstandingSettings();
              requestedSettings[label] =
                {
                  video_understanding_provider: videoSettings.provider,
                  video_understanding_base_url: videoSettings.baseUrl,
                  video_understanding_model: videoSettings.model,
                  video_understanding_api_key: videoSettings.apiKey,
                }[label] || null;
              break;
            }
            default:
      ...
      try {
        ...
        const settings = {
          ...
          video_understanding_enabled:
            (await SystemSettings.get({ label: "video_understanding_enabled" }))
              ?.value || "false",
  ...
  app.post(
    "/admin/video-understanding/test",
    [validatedRequest, flexUserRoleValid([ROLES.admin, ROLES.manager])],
    async (request, response) => {
      try {
        const {
        ...
        const result = await testVideoUnderstandingConnection(reqBody(request));
        response.status(200).json(result);
      } catch (e) {
        console.error(e);
  ...
  app.post(
    ...
    async (request, response) => {
      try {
        ...
        await EventLogs.logEvent(
          "api_key_created",
          { createdBy: user?.username, keyName: name },
          user?.id
        );
        return response.status(200).json({
          apiKey,
   ...
   */
  app.get(
    "/admin/knowledge-graph/settings",
    [validatedRequest, flexUserRoleValid([ROLES.admin, ROLES.manager])],
    async (_request, response) => {
      try {
        const {
        ...
        const [flags, degradation] = await Promise.all([
          getFeatureFlags(),
          getDegradationConfig(),
        ]);
        ...
        response.status(200).json({
          settings: {
            // 功能开关
            guidedRetrievalEnabled: flags.GUIDED_RETRIEVAL_ENABLED,
            entityExtractionEnabled: flags.ENTITY_EXTRACTION_ENABLED,
            similarityEdgesEnabled: flags.SIMILARITY_EDGES_ENABLED,
            pathFinderEnabled: flags.PATH_FINDER_ENABLED,
            // 性能配置
            searchTimeoutMs: degradation.SEARCH_TIMEOUT_MS,
            circuitBreakerThreshold: degradation.CIRCUIT_BREAKER_THRESHOLD,
          },
          status: {
            circuitBreaker: {
        ...
        console.error("[Admin] Failed to get KG settings:", e);
        response.status(500).json({
          settings: null,
          error: "Failed to get knowledge graph settings",
        });
      }
    }
  );
  ...
  app.post(
    ...
    async (request, response) => {
      try {
        ...
        // 映射前端字段到数据库字段
        const dbUpdates = {};
        if (updates.guidedRetrievalEnabled !== undefined) {
          dbUpdates.kg_guided_retrieval_enabled =
            updates.guidedRetrievalEnabled;
        }
        if (updates.entityExtractionEnabled !== undefined) {
          dbUpdates.kg_entity_extraction_enabled =
            updates.entityExtractionEnabled;
        }
        if (updates.similarityEdgesEnabled !== undefined) {
          dbUpdates.kg_similarity_edges_enabled =
            updates.similarityEdgesEnabled;
        }
        if (updates.pathFinderEnabled !== undefined) {
          dbUpdates.kg_path_finder_enabled = updates.pathFinderEnabled;
        }
        if (updates.searchTimeoutMs !== undefined) {
          dbUpdates.kg_search_timeout_ms = updates.searchTimeoutMs;
        }
        if (updates.circuitBreakerThreshold !== undefined) {
          dbUpdates.kg_circuit_breaker_threshold =
            updates.circuitBreakerThreshold;
        }

        // 更新设置
        await SystemSettings.updateSettings(dbUpdates);

        // 清除 featureFlags 缓存，使新设置立即生效
        clearDbSettingsCache();

        await EventLogs.logEvent(
          "knowledge_graph_settings_updated",
          {
            updatedBy: response.locals?.user?.username,
        ...
        console.error("[Admin] Failed to update KG settings:", e);
        response.status(500).json({
          success: false,
          error: "Failed to update knowledge graph settings",
        });
      }
    }
  );
   ...
   */
  app.post(
    "/admin/knowledge-graph/reset-circuit-breaker",
    [validatedRequest, flexUserRoleValid([ROLES.admin])],
    async (_request, response) => {
      try {
        const { recordSuccess } = require("../utils/graphBuilder/featureFlags");
        recordSuccess(); // 重置熔断器状态

        await EventLogs.logEvent(
          "knowledge_graph_circuit_breaker_reset",
          { resetBy: response.locals?.user?.username },
          response?.locals?.user?.id
        );

        response.status(200).json({ success: true, error: null });
      } catch (e) {
        console.error("[Admin] Failed to reset circuit breaker:", e);
        response.status(500).json({
          success: false,
