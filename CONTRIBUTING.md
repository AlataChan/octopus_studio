# Contributing to Octopus Studio

Octopus Studio is an open-source project and we welcome contributions from the community.

> **Upstream note**  
> Octopus Studio is a derivative work based on the MIT-licensed project [AnythingLLM](https://github.com/mintplex-labs/anything-llm) by Mintplex Labs.  
> If you wish to contribute directly to the upstream project, please use the AnythingLLM repository. This document describes how to contribute to **this** repository (`AlataChan/octopus_studio`).

## Reporting Issues

If you encounter a bug or have a feature request, please open an issue on the
[GitHub issue tracker](https://github.com/AlataChan/octopus_studio/issues).

## Picking an issue

We track issues on the GitHub issue tracker. If you are looking for something to
work on, check the [good first issue](https://github.com/AlataChan/octopus_studio/contribute) label. These issues are typically the best described and have the smallest scope. There may be issues that are not labeled as good first issue, but are still a good starting point.
...
1. If you are modifying the permission system for a new role or something custom, you are likely better off forking the project and building your own version since this is a core part of the project and is only to be maintained by the Octopus Studio maintainers.
...
4. **Security** is always important. If you have a security concern:
   - For **Octopus Studio**, please open a security-focused issue in this repository (you can mark it clearly as `[security]`) so maintainers can triage it.
   - For issues that affect the upstream AnythingLLM project, please follow the upstream security process described in their repository.
...
```bash
git clone https://github.com/<username>/octopus_studio.git
cd octopus_studio
```
...
```bash
git remote add upstream https://github.com/AlataChan/octopus_studio.git
git fetch upstream
```
...
For Octopus Studio, these components are included as submodules/subpackages and follow the upstream implementation.
...
Changes to the core Octopus Studio project are released through the `main` branch of this repository. When a PR is merged into `main`, a new image or build may be produced for internal or external deployment flows.

When a new version is released, the following steps are taken:

- A new image or build is created based on the current `master` branch
- Version tags follow the format `v<major>.<minor>.<patch>` and are pinned to specific commits, while `latest` refers to the most recent stable release (if used in your deployment flow)

### Desktop propogation

If you maintain a desktop or Electron-based distribution of Octopus Studio, it will typically consume the core web/server code from this repository as a downstream dependency. That distribution is **not** part of the core Octopus Studio project unless explicitly stated, and may be maintained separately.

## License

By contributing to Octopus Studio (this repository), you agree to license your contributions under the MIT license, consistent with the upstream AnythingLLM project license.
